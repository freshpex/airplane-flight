...
const ManageBooking = () => {
  const [searchType, setSearchType] = useState<'booking-reference' | 'ticket-number'>('booking-reference');
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (searchType === 'booking-reference') {
        if (reference.toUpperCase() === mockBooking.bookingReference && 
            lastName.toLowerCase() === mockBooking.passengers[0].name.split(' ')[1].toLowerCase()) {
          setBookingData(mockBooking);
        } else {
          setError('No booking found with the provided reference and last name.');
        }
      } else {
        // For the demo, we'll just check if the ticket number has a specific format
        if (ticketNumber.match(/^\d{13}$/) && 
            lastName.toLowerCase() === mockBooking.passengers[0].name.split(' ')[1].toLowerCase()) {
          setBookingData(mockBooking);
        } else {
          setError('No booking found with the provided ticket number and last name.');
        }
      }
      setLoading(false);
    }, 1500);
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        >
          <Tabs value={searchType} onValueChange={(value) => setSearchType(value as 'booking-reference' | 'ticket-number')}>
              ...
              <TabsContent value="ticket-number" className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      Your e-ticket number is a 13-digit number found on your e-ticket.
                    </p>
                  </div>
                  <div>
        ...
        >
          {error && (
            <div className="bg-white rounded-xl p-6 shadow-lg mb-6 flex items-start gap-4">
              ...
              <div>
                <h3 className="text-lg font-semibold mb-1">Booking Not Found</h3>
                <p className="text-gray-600">{error}</p>
                <p className="text-gray-600 mt-2">
                  Please check your information and try again, or contact our customer service for assistance.
                </p>
              </div>
            </div>
          )}
          ...
          {bookingData && (
            <div className="space-y-6">
              ...
              >
                <div className="bg-purple-600 p-4 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xl font-bold text-white">Booking {bookingData.bookingReference}</h3>
                  </div>
                ...
                <div className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      <div className="space-y-2">
                        {bookingData.passengers.map((passenger: any, index: number) => (
                          <div key={index} className="flex items-center gap-2">
                            ...
                            <span className="text-gray-800">{passenger.name} ({passenger.type})</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-gray-800">
                        <div className="text-lg font-bold">{bookingData.payment.totalPaid}</div>
                        <div className="text-sm">{bookingData.payment.method}</div>
                        <div className="text-sm text-gray-500">Paid on {bookingData.payment.date}</div>
                      </div>
                    </div>
                  </div>
              ...
              {/* Flight Details */}
              <h3 className="text-xl font-semibold mt-8 mb-4">Flight Details</h3>
                  ...
                  <div className="bg-gray-50 p-4 border-b border-gray-200 flex flex-wrap justify-between items-center">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <span className="ml-2 text-gray-700">{flight.departureDate}</span>
                      </div>
                    </div>
                  ...
                  <div className="p-5">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                      ...
                      <div className="flex flex-col justify-center items-center">
                        ...
                        <div className="text-sm text-gray-700 mt-2">{flight.aircraft}</div>
                        <div className="text-sm text-purple-600 font-medium">{flight.cabin}</div>
                      </div>
                    ...
                    <div className="flex justify-between border-t border-gray-100 pt-4">
                      ...
                      <div>
                        {new Date(flight.departureDate) > new Date() ? (
                          ...
                          <Button variant="outline" size="sm" className="border-gray-400 text-gray-400" disabled>
                            Completed
                          </Button>
                        )}
                      </div>
              ...
              {/* Services */}
              <h3 className="text-xl font-semibold mt-8 mb-4">Included Services</h3>
              <motion.div 
              ...
              >
                <div className="space-y-4">
                  {bookingData.services.map((service: any, index: number) => (
                    <div key={index} className="flex items-start justify-between pb-4 border-b border-gray-100 last:border-b-0 last:pb-0">
                      <div className="flex items-start gap-3">
                        <div className={`p-2 rounded-full ${service.included ? 'bg-green-100' : 'bg-gray-100'}`}>
                          {service.type === 'Baggage' && <Plane className={`h-5 w-5 ${service.included ? 'text-green-600' : 'text-gray-600'}`} />}
                          {service.type === 'Meal' && <Users className={`h-5 w-5 ${service.included ? 'text-green-600' : 'text-gray-600'}`} />}
                          {service.type === 'Seat Selection' && <MapPin className={`h-5 w-5 ${service.included ? 'text-green-600' : 'text-gray-600'}`} />}
                        </div>
                        <div>
          ...
          {!bookingData && !error && (
            <div className="bg-white rounded-xl p-8 shadow-lg mb-6 text-center">
              ...
              >
                ...
                <h3 className="text-xl font-semibold mb-2">Manage Your Booking</h3>
                <p className="text-gray-600 max-w-md mx-auto">
                  Enter your booking details to view, modify, or manage your reservation.
                </p>
              </motion.div>
            </div>
          )}
