...
const CheckIn = () => {
  ...
  const [checkInStatus, setCheckInStatus] = useState<'not-started' | 'in-progress' | 'completed' | 'error'>('not-started');
  ...
  const handleCheckInSearch = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (reference.toUpperCase() === mockFlight.bookingReference && 
          lastName.toLowerCase() === mockFlight.passengers[0].name.split(' ')[1].toLowerCase()) {
        setFlightDetails(mockFlight);
        setCheckInStatus('in-progress');
        setStep(2);
      } else {
        setError('No booking found with the provided reference and last name, or online check-in is not available for this flight.');
  ...
  const handlePassengerDetailsSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      updatedFlightDetails.passengers = updatedFlightDetails.passengers.map((p: any) => ({
        ...p,
        checkedIn: true
      }));
  ...
  const CheckInSteps = () => (
    <div className="flex items-center justify-between max-w-lg mx-auto mb-8">
      {['Search', 'Passenger Details', 'Boarding Pass'].map((stepName, index) => {
        ...
        return (
          <div key={index} className="flex items-center">
            {index > 0 && (
              <div className={`h-0.5 w-full ${isPast ? 'bg-purple-600' : 'bg-gray-300'}`}></div>
            )}
            <div className={`relative flex items-center justify-center rounded-full h-8 w-8 
              ${isActive ? 'bg-purple-600 text-white' : 
                isPast ? 'bg-purple-600 text-white' : 'bg-gray-200 text-gray-600'}`}>
              {isPast ? <Check className="h-4 w-4" /> : stepNumber}
              <span className={`absolute -bottom-6 whitespace-nowrap text-xs 
            ...
            >
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      Your booking reference is a 6-character code found in your confirmation email.
                    </p>
                  </div>
                  <div>
              ...
              {error && (
                <div className="mt-6 bg-red-50 p-4 rounded-lg flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-red-800 font-semibold">Check-in Failed</h4>
            ...
            >
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                <div className="flex flex-wrap justify-between items-center">
                  <div>
                    <h3 className="font-semibold text-lg">{flightDetails.flight.from.city} ({flightDetails.flight.from.code}) to {flightDetails.flight.to.city} ({flightDetails.flight.to.code})</h3>
                    <div className="text-gray-600 flex flex-wrap gap-x-6 mt-1">
                      <span className="flex items-center gap-1">
                        <Calendar className="h-4 w-4" /> {flightDetails.flight.departureDate}
                      </span>
                      <span className="flex items-center gap-1">
                        <Clock className="h-4 w-4" /> {flightDetails.flight.departureTime}
                      </span>
                      <span className="flex items-center gap-1">
                        <Ticket className="h-4 w-4" /> {flightDetails.flight.flightNumber}
                      </span>
                    </div>
                  </div>
              ...
              <form onSubmit={handlePassengerDetailsSubmit}>
                {flightDetails.passengers.map((passenger: any, index: number) => (
                  <div key={index} className="mb-6 pb-6 border-b border-gray-200 last:border-b-0 last:pb-0">
                    <h3 className="text-lg font-semibold mb-4">Passenger {index + 1}: {passenger.name}</h3>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        <Label htmlFor={`seat-${index}`}>Seat Assignment</Label>
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <Button type="button" variant="outline" size="sm" className="border-purple-600 text-purple-600">
                            Change
                          </Button>
                        </div>
                      </div>
          ...
          {step === 3 && flightDetails && (
            ...
            >
                    ...
                    <div className="p-4">
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        <div>
                          ...
                          <div className="font-semibold">{flightDetails.flight.flightNumber}</div>
                        </div>
                        <div>
                          <div className="text-sm text-gray-500">Date</div>
                          <div className="font-semibold">{flightDetails.flight.departureDate}</div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        <div>
                          ...
                          <div className="font-semibold">{flightDetails.flight.from.city} ({flightDetails.flight.from.code})</div>
                          <div className="text-sm">{flightDetails.flight.from.terminal}</div>
                        </div>
                        <div>
                          <div className="text-sm text-gray-500">To</div>
                          <div className="font-semibold">{flightDetails.flight.to.city} ({flightDetails.flight.to.code})</div>
                          <div className="text-sm">{flightDetails.flight.to.terminal}</div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-4 mb-4">
                        <div>
                          ...
                          <div className="font-semibold">{flightDetails.flight.departureTime}</div>
                        </div>
                        <div>
                          <div className="text-sm text-gray-500">Boarding</div>
                          <div className="font-semibold">{flightDetails.flight.boardingTime}</div>
                        </div>
                        <div>
                          <div className="text-sm text-gray-500">Gate Closes</div>
                          <div className="font-semibold">{flightDetails.flight.gateClosesTime}</div>
                        </div>
                      </div>
                      ...
                      <div className="text-sm text-gray-500 text-center">
                        Booking Reference: <span className="font-semibold">{flightDetails.bookingReference}</span>
                      </div>
                    </div>
                  </motion.div>
                ))}
              ...
              <div className="flex flex-wrap justify-center gap-4">
                <Button variant="outline" className="border-purple-600 text-purple-600 flex items-center gap-2">
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm">
                    Online check-in opens 48 hours before departure and closes 90 minutes before departure. 
                    At the airport, check-in counters close 60 minutes before departure for international flights.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm">
                    For international travel, ensure you have a valid passport with at least 6 months validity. 
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm">
                    ...
                    Business and First Class passengers are allowed 32kg checked baggage. 
                    Maximum dimensions per bag: 158cm (length + width + height).
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm">
                    Economy passengers are allowed one piece up to 7kg. 
                    Business and First Class passengers are allowed two pieces up to 7kg each. 
                    Maximum dimensions: 55 x 40 x 20cm.
                  </p>
                </div>
              </div>
            </div>
