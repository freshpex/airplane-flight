...
interface FlightInfo {
  ...
  status: 'SCHEDULED' | 'ON_TIME' | 'DELAYED' | 'BOARDING' | 'DEPARTED' | 'IN_AIR' | 'LANDED' | 'ARRIVED' | 'CANCELLED' | 'DIVERTED';
  aircraft?: string;
  duration?: string;
}
      ...
      <Tabs defaultValue="all">
        ...
        <TabsContent value="departures">
          {flights.length > 0 ? (
            <div className="space-y-4">
              {flights.map((flight, index) => (
                <FlightCard key={`${flight.flightNumber}-${index}-dep`} flight={flight} />
              ))}
            </div>
          ) : (
            <div className="text-center py-12 text-gray-500">
        ...
        <TabsContent value="arrivals">
          {flights.length > 0 ? (
            <div className="space-y-4">
              {flights.map((flight, index) => (
                <FlightCard key={`${flight.flightNumber}-${index}-arr`} flight={flight} />
              ))}
            </div>
          ) : (
            <div className="text-center py-12 text-gray-500">
      ...
      <CardContent className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            {flight.duration && (
              <p className="text-sm font-medium mt-2">Duration: {flight.duration}</p>
            )}
          </div>
...
function getStatusBadge(status: FlightInfo['status']) {
  const statusMap: Record<FlightInfo['status'], { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline' }> = {
