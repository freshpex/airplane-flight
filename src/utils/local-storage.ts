export const setWithExpiry = (key: string, value: any, ttl: number) => {
  ...
  if (typeof window === "undefined" || typeof localStorage === "undefined") {
    console.warn(
      "[setWithExpiry] localStorage not available (SSR or browser restriction)",
    );
    return false;
  }
...
export const removeWithExpiry = (key: string) => {
  ...
  if (typeof window === "undefined" || typeof localStorage === "undefined") {
    console.warn(
      "[removeWithExpiry] localStorage not available (SSR or browser restriction)",
    );
    return false;
  }
  ...
  try {
    ...
    console.debug(
      `[removeWithExpiry] Successfully removed item for key: ${key}`,
    );
    return true;
  } catch (e) {
...
export const getWithExpiry = (key: string) => {
  ...
  if (typeof window === "undefined" || typeof localStorage === "undefined") {
    console.warn(
      "[getWithExpiry] localStorage not available (SSR or browser restriction)",
    );
    return null;
  }
  ...
  try {
    ...
    console.debug(
      `[getWithExpiry] Successfully retrieved item for key: ${key}`,
    );
    return item.value;
  } catch (e) {
    // If there's any error in parsing, remove the item and return null
    console.error(
      `[getWithExpiry] Error accessing localStorage for key ${key}:`,
      e,
    );
    try {
      localStorage.removeItem(key);
      ...
      console.error(
        `[getWithExpiry] Error removing invalid item for key ${key}:`,
        removeError,
      );
    }
    return null;
